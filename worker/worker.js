...
// `Authorization: Bearer <secret>` — additive; the legacy ?secret= /
// body.secret / webhook-path transports keep working unchanged.
//
...
async function resolveAuth(env, request, url) {
  ...
  // v9.1.0: the shared secret may also arrive as a bare Bearer header
  // (no X-Device-Id). Device authTokens never match CONFIG.secret, so
  // this only fires for the legacy shared secret on the new transport;
  // it makes per-user rate limiting attribute Bearer-secret callers to
  // the admin identity exactly like ?secret= callers.
  const bearerSecret = (!deviceId && authHeader.startsWith('Bearer ')) ? authHeader.slice(7).trim() : '';
  if ((providedSecret && await checkSecret(env, providedSecret))
      || (bearerSecret && await checkSecret(env, bearerSecret))) {
...
// ════════════════════════════════════════════════════════════════════
// === v9.1.0 — Shared-secret auth wrapper + route table ===
// ════════════════════════════════════════════════════════════════════
//
// The shared secret is accepted from, per route:
//   - ?secret= query param          (legacy — every deployed device)
//   - body.secret / body.oldSecret  (routes that always read it there)
//   - the /webhook/{secret} path segment
// and, NEW in v9.1.0, uniformly on every shared-secret route:
//   - Authorization: Bearer <secret>
// The Bearer transport is additive and backward compatible: a Bearer
// header that carries a device authToken (not the shared secret) just
// fails the string comparison below and the route 403s exactly as it
// did before; device-token auth stays in resolveAuth (which requires
// the X-Device-Id header alongside the Bearer token).
//
// SECURITY NOTE: never log the candidate secrets, the Authorization
// header, or (for /webhook/*) the raw path — the webhook secret is a
// path segment.
...
// The single shared-secret gate. `candidates` carries the per-route
// legacy transports (query param, body field, or webhook path segment);
// the Bearer header is accepted for every shared-secret route.
async function checkSharedSecretAuth(env, request, candidates) {
  const real = await env.CONFIG.get('secret');
  if (!real) return false;
  for (const c of candidates) {
    if (c != null && c === real) return true;
  }
  const bearer = getBearerToken(request);
  return !!(bearer && bearer === real);
}
...
// Body-parse failure responses for routes whose secret arrives in the
// JSON body. Each matches the response the route's old catch-block
// produced when `await request.json()` threw.
function badJsonResponse(kind, e) {
  if (kind === 'json502') return jsonResponse({ error: e.message }, 502);
  if (kind === 'json500') return jsonResponse({ error: e.message }, 500);
...
// ════════════════════════════════════════════════════════════════════
//
// Every handler has the signature (request, env, url, ctx) where ctx
// carries what the dispatch loop resolved for it:
//   ctx.body — parsed JSON body (secret-body routes only)
//   ctx.auth — { userId, deviceId, viaLegacy } (bearer/admin routes only)
...
async function handleEventsList(request, env, url, ctx) {
...
async function handleEventsAck(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
async function handleMetadataLookup(request, env, url, ctx) {
...
async function handleMetadataBulk(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
async function handleViewedList(request, env, url, ctx) {
...
async function handlePromotionsAdd(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
async function handlePromotionsList(request, env, url, ctx) {
...
async function handlePromotionsDelete(request, env, url, ctx) {
...
// }
async function handlePalateUpsert(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
async function handlePalateArchived(request, env, url, ctx) {
...
async function handlePalateList(request, env, url, ctx) {
...
async function handlePalatePredictTags(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
async function handlePlexConfigure(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
async function handlePlexScrobble(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
// auth: custom — resolveAuth picks bearer vs legacy per-request.
async function handleSyncGet(request, env, url, ctx) {
  const auth = await resolveAuth(env, request, url);
  let key = null;
  if (auth?.userId && !auth.viaLegacy) {
    ...
    if (!(await checkSharedSecretAuth(env, request, [providedSecret]))) return forbidden();
...
// auth: custom — same split as /sync/get.
async function handleSyncPut(request, env, url, ctx) {
  const auth = await resolveAuth(env, request, url);
  let key = null;
  if (auth?.userId && !auth.viaLegacy) {
    key = `state:${auth.userId}`;
...
async function handleAlertsSubscribe(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
async function handleAlertsVapidPublic(request, env, url, ctx) {
...
async function handleAlertsUnsubscribe(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
async function handleAlertsStatus(request, env, url, ctx) {
  const userHash = url.searchParams.get('user');
...
async function handleAlertsNotifications(request, env, url, ctx) {
  const userHash = url.searchParams.get('user');
  const since = parseInt(url.searchParams.get('since') || '0');
  if (!userHash) return new Response('Missing user', { status: 400, headers: cors });
...
async function handleAlertsNotificationsSeen(request, env, url, ctx) {
  try {
    const body = ctx.body;
    if (!body.userHash) return new Response('Missing userHash', { status: 400, headers: cors });
...
async function handleCronBackupState(request, env, url, ctx) {
...
async function handleCronMigrateViewedToD1(request, env, url, ctx) {
  try {
    const cursor = url.searchParams.get('cursor') || undefined;
...
async function handleCronCheckAlerts(request, env, url, ctx) {
...
// auth: custom — tolerant body parse, secret from body.oldSecret or query.
async function handleMigrate(request, env, url, ctx) {
  try {
    const body = await request.json().catch(() => ({}));
    const providedSecret = body.oldSecret || url.searchParams.get('secret');
    if (!(await checkSharedSecretAuth(env, request, [providedSecret]))) {
      return forbidden();
    }
...
// Devices cards.
async function handleUserMe(request, env, url, ctx) {
  const auth = ctx.auth;
  const row = await env.D1_VIEWED.prepare(
...
async function handleAdminInvitesCreate(request, env, url, ctx) {
  const auth = ctx.auth;
  const body = await request.json().catch(() => ({}));
...
async function handleAdminInvitesList(request, env, url, ctx) {
  const out = [];
  let cursor = undefined;
  do {
...
async function handleAdminInvitesDelete(request, env, url, ctx) {
  const path = url.pathname;
...
// /pair/begin/owned requires bearer auth (checked inside — the plain
// variant is public, so the table can't gate it).
async function handlePairBegin(request, env, url, ctx) {
  const owned = url.pathname === '/pair/begin/owned';
...
async function handlePairInfo(request, env, url, ctx) {
...
async function handlePairConfirm(request, env, url, ctx) {
  const auth = ctx.auth;
  const body = await request.json().catch(() => ({}));
  ...
  if (rec.ownedByUserId && rec.ownedByUserId !== auth.userId) {
  ...
  rec.consumed = true;
  rec.consumedAt = now;
  rec.fulfillment = { newDeviceId, userId: auth.userId };
...
async function handlePairWait(request, env, url, ctx) {
  const sessionId = url.searchParams.get('session');
...
async function handleAdminReconnectCreate(request, env, url, ctx) {
  const auth = ctx.auth;
  const body = await request.json().catch(() => ({}));
...
// is preserved.
async function handleReconnect(request, env, url, ctx) {
  try {
    const body = await request.json().catch(() => ({}));
...
async function handleDeviceRename(request, env, url, ctx) {
  const auth = ctx.auth;
  const path = url.pathname;
  ...
  if (rec.userId !== auth.userId) return jsonResponse({ error: 'not your device' }, 403);
  const body = await request.json().catch(() => ({}));
...
async function handleDeviceDelete(request, env, url, ctx) {
  const auth = ctx.auth;
  const path = url.pathname;
...
async function handleAdminUserRevoke(request, env, url, ctx) {
  const path = url.pathname;
...
async function handleAdminUsersList(request, env, url, ctx) {
...
async function handleAdminUserRename(request, env, url, ctx) {
  const path = url.pathname;
...
async function handleRegister(request, env, url, ctx) {
  try {
    const body = await request.json().catch(() => ({}));
...
async function handleUserSettings(request, env, url, ctx) {
  const auth = ctx.auth;
  const body = await request.json().catch(() => ({}));
  const updates = [];
  const binds = [];
...
async function handleBootstrapStatus(request, env, url, ctx) {
...
async function handleBootstrapCredentials(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
async function handleTraktDeviceCodeInit(request, env, url, ctx) {
...
async function handleTraktDeviceCodePoll(request, env, url, ctx) {
  try {
    const body = ctx.body;
...
async function handleTraktScrobble(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
...
async function handleTraktUnwatch(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
  if (!userId) return jsonResponse({ error: 'user required' }, 400);
  try {
    const body = await request.json();
...
async function handleTraktRate(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
  if (!userId) return jsonResponse({ error: 'user required' }, 400);
  try {
    const body = await request.json();
...
async function handleTraktUnrate(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
  if (!userId) return jsonResponse({ error: 'user required' }, 400);
  try {
    const body = await request.json();
...
async function handleTraktWatchlistAdd(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
  if (!userId) return jsonResponse({ error: 'user required' }, 400);
  try {
    const body = await request.json();
...
async function handleTraktHistory(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
  if (!userId) return jsonResponse({ error: 'user required' }, 400);
...
async function handleTraktRatings(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
  if (!userId) return jsonResponse({ error: 'user required' }, 400);
  const type = url.searchParams.get('type') === 'shows' ? 'shows' : 'movies';
  try {
...
async function handleTraktMe(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
  if (!userId) return jsonResponse({ error: 'user required' }, 400);
  try {
...
async function handleTraktDisconnect(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
  if (!userId) return jsonResponse({ error: 'user required' }, 400);
  if (!env.D1_VIEWED) return jsonResponse({ error: 'D1_VIEWED binding missing' }, 500);
  try {
...
async function handleWatchMark(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
  if (!userId) return jsonResponse({ error: 'user required' }, 400);
  try {
    const body = await request.json();
...
async function handleWatchHistory(request, env, url, ctx) {
  const userId = url.searchParams.get('user');
  if (!userId) return jsonResponse({ error: 'user required' }, 400);
  if (!env.D1_VIEWED) return jsonResponse({ error: 'D1_VIEWED binding missing' }, 500);
  const since = parseInt(url.searchParams.get('since') || '0');
...
// ════════════════════════════════════════════════════════════════════
//
// One route per line; the dispatch loop walks the table IN ORDER, so
// entries mirror the order of the old if/else chain (order only matters
// where a prefix could shadow a later entry with the same method).
//
// Matching: `path` is an exact-match string; `prefix`/`suffix` use
// path.startsWith / path.endsWith. Parameterized segments (e.g.
// /promotions/{tab}/{itemId}, /webhook/{secret}, /devices/{id}/rename)
// are extracted inside the handler, exactly as before.
//
// auth values:
//   'none'              — public route
//   'secret'            — shared secret via ?secret= or Bearer
//   'secret-body'       — shared secret via body.secret or Bearer; the
//                          JSON body is parsed by the dispatch loop and
//                          passed as ctx.body; a parse failure returns
//                          badJson ('text400' default | 'json502' | 'json500')
//   'secret-body-query' — body.secret || ?secret= (legacy `||` semantics)
//                          or Bearer; body parsed like 'secret-body'
//   'webhook'           — shared secret via the /webhook/{secret} path
//                          segment or Bearer
//   'bearer'            — device bearer auth (requireBearer → ctx.auth)
...
const ROUTES = [
  ...
  { method: 'GET',    path: '/promotions',                          auth: 'secret',            handler: handlePromotionsList },
  { method: 'DELETE', prefix: '/promotions/',                       auth: 'secret',            handler: handlePromotionsDelete },
  { method: 'POST',   path: '/palate/upsert',                       auth: 'secret-body',       handler: handlePalateUpsert },
  { method: 'GET',    path: '/palate/archived',                     auth: 'secret',            handler: handlePalateArchived },
  { method: 'GET',    path: '/palate/list',                         auth: 'secret',            handler: handlePalateList },
  { method: 'POST',   path: '/palate/predict-tags',                 auth: 'secret-body',       handler: handlePalatePredictTags },
  { method: 'POST',   path: '/plex/configure',                      auth: 'secret-body',       handler: handlePlexConfigure },
  { method: 'GET',    path: '/plex/identity',                       auth: 'secret',            handler: handlePlexIdentity },
  ...
  { method: 'GET',    path: '/alerts/vapid-public',                 auth: 'none',              handler: handleAlertsVapidPublic },
  { method: 'POST',   path: '/alerts/unsubscribe',                  auth: 'secret-body',       handler: handleAlertsUnsubscribe },
  { method: 'GET',    path: '/alerts/status',                       auth: 'secret',            handler: handleAlertsStatus },
  { method: 'GET',    path: '/alerts/notifications',                auth: 'secret',            handler: handleAlertsNotifications },
  { method: 'POST',   path: '/alerts/notifications/seen',           auth: 'secret-body',       handler: handleAlertsNotificationsSeen },
  { method: 'GET',    path: '/alerts/test-fire',                    auth: 'secret',            handler: handleAlertsTestFire },
  { method: 'GET',    path: '/cron/backup-state',                   auth: 'secret',            handler: handleCronBackupState },
  { method: 'GET',    path: '/cron/migrate-viewed-to-d1',           auth: 'secret',            handler: handleCronMigrateViewedToD1 },
  { method: 'POST',   path: '/chat',                                auth: 'secret-body',       handler: handleChat, badJson: 'json500' },
  { method: 'GET',    path: '/cron/check-alerts',                   auth: 'secret',            handler: handleCronCheckAlerts },
  { method: 'POST',   path: '/migrate',                             auth: 'custom',            handler: handleMigrate },
  { method: 'GET',    path: '/user/me',                             auth: 'bearer',            handler: handleUserMe },
  { method: 'POST',   path: '/admin/invites/create',                auth: 'admin',             handler: handleAdminInvitesCreate },
  { method: 'GET',    path: '/admin/invites/list',                  auth: 'admin',             handler: handleAdminInvitesList },
  { method: 'DELETE', prefix: '/admin/invites/',                    auth: 'admin',             handler: handleAdminInvitesDelete },
  { method: 'POST',   path: '/pair/begin',                          auth: 'none',              handler: handlePairBegin },
  { method: 'POST',   path: '/pair/begin/owned',                    auth: 'none',              handler: handlePairBegin },
  { method: 'GET',    path: '/pair/info',                           auth: 'none',              handler: handlePairInfo },
  { method: 'POST',   path: '/pair/confirm',                        auth: 'bearer',            handler: handlePairConfirm },
  { method: 'GET',    path: '/pair/wait',                           auth: 'none',              handler: handlePairWait },
  { method: 'POST',   path: '/admin/reconnect/create',              auth: 'admin',             handler: handleAdminReconnectCreate },
  { method: 'POST',   path: '/reconnect',                           auth: 'none',              handler: handleReconnect },
  { method: 'POST',   prefix: '/devices/', suffix: '/rename',       auth: 'bearer',            handler: handleDeviceRename },
  { method: 'DELETE', prefix: '/devices/',                          auth: 'bearer',            handler: handleDeviceDelete },
  { method: 'POST',   prefix: '/admin/users/', suffix: '/revoke',   auth: 'admin',             handler: handleAdminUserRevoke },
  { method: 'GET',    path: '/admin/users/list',                    auth: 'admin',             handler: handleAdminUsersList },
  { method: 'POST',   prefix: '/admin/users/', suffix: '/rename',   auth: 'admin',             handler: handleAdminUserRename },
  { method: 'POST',   path: '/register',                            auth: 'none',              handler: handleRegister },
  { method: 'PUT',    path: '/user/settings',                       auth: 'bearer',            handler: handleUserSettings },
  { method: 'GET',    path: '/bootstrap/status',                    auth: 'secret',            handler: handleBootstrapStatus },
  { method: 'POST',   path: '/bootstrap/credentials',               auth: 'secret-body',       handler: handleBootstrapCredentials, badJson: 'json500' },
  { method: 'POST',   path: '/api/trakt/device-code-init',          auth: 'secret',            handler: handleTraktDeviceCodeInit },
  { method: 'POST',   path: '/api/trakt/device-code-poll',          auth: 'secret-body-query', handler: handleTraktDeviceCodePoll, badJson: 'json502' },
  { method: 'POST',   path: '/api/trakt/scrobble',                  auth: 'secret',            handler: handleTraktScrobble },
  { method: 'POST',   path: '/api/trakt/unwatch',                   auth: 'secret',            handler: handleTraktUnwatch },
  { method: 'POST',   path: '/api/trakt/rate',                      auth: 'secret',            handler: handleTraktRate },
  { method: 'POST',   path: '/api/trakt/unrate',                    auth: 'secret',            handler: handleTraktUnrate },
  { method: 'POST',   path: '/api/trakt/watchlist-add',             auth: 'secret',            handler: handleTraktWatchlistAdd },
  { method: 'GET',    path: '/api/trakt/history',                   auth: 'secret',            handler: handleTraktHistory },
  { method: 'GET',    path: '/api/trakt/ratings',                   auth: 'secret',            handler: handleTraktRatings },
  { method: 'GET',    path: '/api/trakt/me',                        auth: 'secret',            handler: handleTraktMe },
...
function routeMatches(route, method, path) {
  ...
  if (route.prefix !== undefined && !path.startsWith(route.prefix)) return false;
  if (route.suffix !== undefined && !path.endsWith(route.suffix)) return false;
  return true;
}
...
export default {
  async fetch(request, env) {
    ...
    // Never log the raw path for /webhook/* — the shared secret is a
    // path segment there.
    const logPath = path.startsWith('/webhook/') ? '/webhook/<redacted>' : path;
    ...
    // routes are unauthenticated); route-level auth is enforced by the
    // dispatch loop below.
    const ip = request.headers.get('CF-Connecting-IP') || request.headers.get('x-forwarded-for') || '';
    const rl = await checkRateLimit(env, ip);
    if (!rl.ok) {
      console.log('[rate-limit/ip] 429 for ip', ip, 'on', method, logPath, '—', rl.count, '/', rl.limit);
    ...
    if (preAuth?.userId) {
      ...
      if (!userRl.ok) {
        console.log('[rate-limit/user] 429 for', preAuth.userId, 'bucket', bucketForPath(path), 'on', method, logPath);
    ...
    // Route dispatch — first matching entry wins; auth is enforced here
    // so handlers never re-check the shared secret themselves.
    for (const route of ROUTES) {
      if (!routeMatches(route, method, path)) continue;
      const ctx = {};
      switch (route.auth) {
        case 'none':
        case 'custom':
          break;
        case 'secret': {
          if (!(await checkSharedSecretAuth(env, request, [url.searchParams.get('secret')]))) return forbidden();
          break;
        }
        case 'secret-body':
        case 'secret-body-query': {
          let body;
          try { body = await request.json(); }
          catch (e) { return badJsonResponse(route.badJson, e); }
          const candidates = route.auth === 'secret-body-query'
            ? [body.secret || url.searchParams.get('secret')]
            : [body.secret];
          if (!(await checkSharedSecretAuth(env, request, candidates))) return forbidden();
          ctx.body = body;
          break;
        }
        case 'webhook': {
          if (!(await checkSharedSecretAuth(env, request, [path.slice('/webhook/'.length)]))) return forbidden();
          break;
        }
        case 'bearer': {
          const r = await requireBearer(env, request, url);
      ...
      return route.handler(request, env, url, ctx);
    }
