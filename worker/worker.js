...
Only predict tags that are in the provided tag set for that item's content type. Predict 2-5 tags per item. Prioritize tags that are most likely to be accurate over completeness.`;
    ...
    // === Per-item SRR state in D1 (dual-write phase) ===
    //
    // Successor to the /sync blob for Status/Rating/Reaction data: one
    // D1 `item_state` row per (user, item), conflict-resolved per item
    // by last_updated instead of last-write-wins on the whole blob.
    // While dual-write is in effect the SYNC_KV blob stays canonical —
    // the client shadow-reads these routes but does not apply results.
    // Cutover steps: docs/SRR-D1-CUTOVER.md. Schema: migrations/
    // 004_item_state.sql. Auth matches the legacy /sync routes:
    // shared secret + SHA-256(plex_token) user hash.

    // GET /state/tab?secret=X&user=HASH&tab=T — all rows for one tab.
    if (path === '/state/tab' && method === 'GET') {
      const providedSecret = url.searchParams.get('secret');
      if (!(await checkSecret(env, providedSecret))) return new Response('Forbidden', { status: 403, headers: cors });
      const userHash = url.searchParams.get('user');
      if (!userHash || !/^[a-f0-9]{16,64}$/i.test(userHash)) {
    ...
    if (path === '/state/upsert' && method === 'POST') {
      try {
        const body = await request.json();
        if (!(await checkSecret(env, body.secret))) return new Response('Forbidden', { status: 403, headers: cors });
    ...
    if (path === '/state/counts' && method === 'GET') {
      const providedSecret = url.searchParams.get('secret');
      if (!(await checkSecret(env, providedSecret))) return new Response('Forbidden', { status: 403, headers: cors });
      const userHash = url.searchParams.get('user');
      if (!userHash || !/^[a-f0-9]{16,64}$/i.test(userHash)) {
    ...
    if (path === '/cron/migrate-state-to-d1' && method === 'POST') {
      const providedSecret = url.searchParams.get('secret');
      if (!(await checkSecret(env, providedSecret))) return new Response('Forbidden', { status: 403, headers: cors });
      try {
